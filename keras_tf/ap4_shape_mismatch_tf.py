...
def _pad_and_cast_tf(x_batch: tf.Tensor, y_batch: tf.Tensor):
    ...
    # Ensure inputs are float32 — tf.data can yield float64 on some runtimes,
    # which causes a dtype mismatch in tensor_scatter_nd_update against the
    # float32 padded_x / padded_y targets.
...
def train_fn(X_train, y_cat_train, X_test, y_cat_test, y_test_raw):
    ...
    # Warm-up: force Adam to create slot variables in eager mode before
    # any @tf.function tracing (fixes Keras 3 singleton tf.Variable error).
    dummy_x = X_train[:1].astype(np.float32)
    dummy_y = y_cat_train[:1].astype(np.float32)
    with tf.GradientTape() as _tape:
        _preds = model(dummy_x, training=True)
        _loss  = loss_fn(dummy_y, _preds)
